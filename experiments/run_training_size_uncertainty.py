...
def main() -> int:
    ...
    for seed in cfg.get("seeds", [0]):
        ...
        for size in sizes:
            ...
            metrics = calibration_metrics(result.summary, truth, top_k=int(cfg.get("top_k", 5)))
            rows.append({**base_metadata(**data.metadata), "method": "bayesian_hyb_fpde", "model": model_name, "training_size": int(size), "status": "ok", "error": "", "sign_confidence": float(np.mean(np.maximum(result.summary["p_positive"], result.summary["p_negative"]))), "rank_stability": float(np.nanmean(result.summary["rank_probability_top_k"])), **metrics})
