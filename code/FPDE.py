...
Clean FPDE core for:
    ...
    - Diff-FPDE
    - Cos-FPDE
    - Hyb-FPDE over lambda_hyb = 0.0, 0.1, ..., 1.0 by default
    - Hyb-FPDE validation-based lambda_hyb selection
...
candidate generator for Hyb-FPDE.
    ...
    *,
    ...
    if mode not in ("diff", "cos"):
        raise ValueError(
            ...
            "or grid_search_hyb_fpde_grid() for Hyb-FPDE"
        )
...
def grid_search_hyb_fpde_grid(
    ...
    *,
    ...
    """Exhaustively search single-prototype Diff/Cos/Hyb-FPDE settings.

    This is the non-MP Hyb-FPDE condition:

        Diff-FPDE: kept as an explicit baseline
        Cos-FPDE : kept as an explicit baseline
        Hyb-FPDE : evaluated for every lambda_hyb_grid value
...
def explain_hyb_fpde_grid(
    ...
    *,
    ...
    """Explain one sample with Diff, Cos, and Hyb-FPDE for all lambdas."""
    X_train_arr = _as_2d_float("X_train", X_train)
    y_train_arr = np.asarray(list(y_train), dtype=object)
    x_arr = _as_1d_float("x", x)
...
def hyb_fpde_grid_attribution(
    ...
    *,
    ...
    """Compute Hyb-FPDE attribution for one fixed lambda_hyb.
...
def select_lambda_by_deletion_insertion_validation(
    ...
    *,
    ...
    This is the default recommended lambda-selection API for Hyb-FPDE.
