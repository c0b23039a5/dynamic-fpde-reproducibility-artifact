...
def main() -> int:
    ...
    for seed in cfg.get("seeds", [0]):
        ...
        for posterior_samples in cfg.get("posterior_samples_grid", [100, 500]):
            ...
            rows.append({**base_metadata(**data.metadata), "ablation": "posterior_samples", "posterior_samples": int(posterior_samples), "lambda_hyb": 0.5, "method": "bayesian_hyb_fpde", "model": model_name, "status": "ok", "error": "", "mean_ci_width": float(np.mean(result.summary["ci_upper_95"] - result.summary["ci_lower_95"])), **metrics})
        for lambda_hyb in cfg.get("lambda_grid", [0.0, 0.5, 1.0]):
            ...
            attr = result.summary["posterior_mean"].to_numpy(dtype=float)
            metrics = deletion_insertion_metrics(model, x, attr, baseline, target_label=result.positive_label)
            rows.append({**base_metadata(**data.metadata), "ablation": "lambda_hyb", "posterior_samples": int(cfg.get("posterior_samples", 100)), "lambda_hyb": float(lambda_hyb), "method": "bayesian_hyb_fpde", "model": model_name, "status": "ok", "error": "", "mean_ci_width": float(np.mean(result.summary["ci_upper_95"] - result.summary["ci_lower_95"])), **metrics})
